...
def test_get_full_state_dict_context_offload(set_type_mock, monkeypatch):
    ...
    # Shards on an accelerator: consolidate to CPU to avoid OOMing the device.
    ...
    # Shards already on CPU: offloading is a no-op at best and triggers a use-after-free
    # (SIGSEGV) in torch's `FlatParamHandle.to_cpu()`, so it must stay disabled.
    module = Mock(spec=FullyShardedDataParallel)

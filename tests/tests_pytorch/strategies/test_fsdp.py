...
class _CPUFSDPTrainableModel(BoringModel):
    ...
    def configure_model(self) -> None:
        if self.layer is None:
            self.layer = torch.nn.Sequential(torch.nn.Linear(32, 8), torch.nn.ReLU(), torch.nn.Linear(8, 2))
        if isinstance(self.layer, FullyShardedDataParallel):
            return
...
def test_fsdp_cpu_trainable(state_dict_type, tmp_path):
    ...
    genuinely FSDP-wrapped, the loss drops sharply over 10 epochs, and the checkpoint written after training reads back
    ...
    trainer = _make_trainer(max_epochs=10)
    trainer.fit(model)
    ...
    resumed_trainer = _make_trainer(max_epochs=11)
